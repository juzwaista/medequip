...
<x-mail::button :url="url('/staff/setup?token='.$token.'&email='.urlencode($email))">
Set Up Account
</x-mail::button>

If you're having trouble clicking the "Set Up Account" button, copy and paste the URL below into your web browser:

<span class="break-all">[{{ url('/staff/setup?token='.$token.'&email='.urlencode($email)) }}]({{ url('/staff/setup?token='.$token.'&email='.urlencode($email)) }})</span>
