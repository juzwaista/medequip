<template>
    ...
    <MainLayout>
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
            <!-- Header -->
            <div class="mb-8">
                <Link href="/products" class="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-blue-600 transition mb-4">
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                    Back to Products
                </Link>
                <div class="flex items-center gap-3 mb-3">
                    <div class="h-11 w-11 bg-indigo-100 rounded-xl flex items-center justify-center">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">Apply for B2B Account</h1>
                        <p class="text-sm text-gray-500">Unlock wholesale pricing, purchase orders, and request-for-quotation.</p>
                    </div>
                </div>
            </div>

            <!-- Benefits -->
            <div class="bg-gradient-to-br from-indigo-50 to-blue-50 border border-indigo-100 rounded-2xl p-5 mb-8">
                <h3 class="text-sm font-bold text-indigo-900 mb-3">What you get with a B2B account:</h3>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div v-for="benefit in benefits" :key="benefit.title" class="flex items-start gap-2.5">
                        <span class="mt-0.5 flex-shrink-0 w-5 h-5 rounded-full bg-indigo-200/60 flex items-center justify-center">
                            <svg class="w-3 h-3 text-indigo-700" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                        </span>
                        <div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sm:p-8">
                <form @submit.prevent="submit" class="space-y-6">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Company / Organization Name <span class="text-red-500">*</span></label>
                        <input type="text" v-model="form.company_name" required placeholder="e.g., Metro Cavite General Hospital"
                            class="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all placeholder-gray-300">
                        <p v-if="form.errors.company_name" class="text-red-600 text-xs mt-1.5">{{ form.errors.company_name }}</p>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Organization Type <span class="text-red-500">*</span></label>
                        <select v-model="form.business_type" required
                            class="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all bg-white">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">SEC / DTI Document <span class="text-gray-400 font-normal">(Optional)</span></label>
