        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="mb-8">
                <div class="flex items-center text-sm text-ink-soft mb-3">
                    ...
                    <span class="text-ink font-medium">Edit product</span>
                </div>
                <h1 class="text-3xl font-bold text-ink">Edit product</h1>
                <p class="text-ink-soft mt-2">Update photos, options, pricing, and stock.</p>
            </div>
            ...
            <form @submit.prevent="submitForm" class="space-y-6">
                ...
                <section class="rounded-card border border-line bg-white shadow-sm p-6 sm:p-8">
                    ...
                    <p class="text-sm text-ink-soft mb-4">Check images to remove, add more files, and pick the main display. At least one image must remain.</p>
                    ...
                    <div v-if="visibleExisting.length" class="flex flex-wrap gap-4 mb-6">
                        <div v-for="img in visibleExisting" :key="img.id" class="text-center">
                            <div class="relative w-28 h-28 rounded-card border border-line overflow-hidden bg-mist">
                                ...
                                <label class="absolute bottom-0 left-0 right-0 bg-black/60 text-white text-xs py-1 cursor-pointer">
                            ...
                            <label class="mt-2 flex items-center justify-center gap-1 text-xs text-ink cursor-pointer">
                                <input v-model="primaryPick" type="radio" :value="`e:${img.id}`" class="text-brand" />
                                Main
                            </label>
                        </div>
                    </div>
                    ...
                    <input
                        ...
                        multiple
                        @change="onNewImages"
                        class="block w-full max-w-md text-sm text-ink-soft file:mr-4 file:py-2 file:px-4 file:rounded-control file:border-0 file:font-semibold file:bg-brand-tint file:text-brand-dark"
                    />
                    <div v-if="newPreviews.length" class="mt-4 flex flex-wrap gap-4">
                        <div v-for="(src, idx) in newPreviews" :key="'n' + idx" class="text-center">
                            ...
                            <label class="mt-2 flex items-center justify-center gap-1 text-xs text-ink cursor-pointer">
                                <input v-model="primaryPick" type="radio" :value="`n:${idx}`" class="text-brand" />
                                Main
                            </label>
                        </div>
                    </div>
                ...
                <section class="rounded-card border border-line bg-white shadow-sm p-6 sm:p-8">
                    ...
                    <div class="flex gap-2 max-w-xl">
                        <input
                            ...
                            class="flex-1 px-4 py-3 border border-line rounded-card font-mono text-sm shadow-sm focus:ring-2 focus:ring-brand/30 focus:border-brand"
                        />
                ...
                <section class="rounded-card border border-line bg-white shadow-sm p-6 sm:p-8">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-ink-soft mt-2">Your internal reference code for this product.</p>
                    </div>
                </section>
                    ...
                    <div class="grid grid-cols-1 gap-5">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <p class="mt-1.5 min-h-[2.5rem] text-xs text-ink-soft">Shown on the product page (e.g. badge for equipment vs consumable).</p>
                            </div>
                        </div>
                        <div v-if="isMedicineCategory" class="rounded-card border border-brand-soft bg-brand-tint/60 p-4">
                            <label class="flex items-start gap-3 cursor-pointer">
                                ...
                                <span class="text-sm font-medium text-ink">Requires a valid prescription</span>
                            </label>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea v-model="fields.description" rows="4" required class="w-full px-4 py-3 border border-line rounded-card" />
                        </div>
                    </div>
                </section>
                                    ...
                                    <tr v-for="(combo, ci) in combinations" :key="combo._label" class="hover:bg-mist/50">
                                        ...
                                        <td class="px-3 py-2">
                                            <input
                                                ...
                                                class="w-full px-2 py-1.5 border border-line rounded-control text-sm focus:ring-1 focus:ring-brand/40 focus:border-brand"
                                            />
                                        </td>
                                        <td class="px-3 py-2">
                                            <input
                                                ...
                                                class="w-full px-2 py-1.5 border border-line rounded-control text-sm focus:ring-1 focus:ring-brand/40 focus:border-brand"
                                            />
                                            <Link v-if="combo.stock === 0" :href="route('owner.procurement.create')" class="block mt-1 text-xs text-brand hover:underline font-bold whitespace-nowrap">
                                                Restock via PO &rarr;
                                            </Link>
                                        </td>
                        ...
                        <div class="rounded-card border border-line p-4 bg-mist/80">
                            ...
                            <div v-if="fields.has_warranty" class="mt-3">
                                ...
                                <input
                                    ...
                                    placeholder="e.g. 12"
                                    required
                                    class="w-full px-4 py-3 border border-line rounded-card bg-white shadow-sm focus:ring-2 focus:ring-brand/30"
                                />
                            </div>
                        </div>
                        <div class="rounded-card border border-line p-4 bg-mist/80">
                            ...
                            <div v-if="fields.has_expiry" class="mt-3 space-y-4">
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input v-model="fields.manufacturing_date" type="date" class="w-full px-4 py-3 border border-line rounded-card bg-white shadow-sm focus:ring-2 focus:ring-brand/30" />
                                    </div>
                                    <div>
                                        <label class="block text-xs font-bold   text-ink-soft mb-1">Expiration Date <span class="text-red-500">*</span></label>
                                        <input v-model="fields.expiry_date" type="date" required class="w-full px-4 py-3 border border-line rounded-card bg-white shadow-sm focus:ring-2 focus:ring-brand/30" />
                                    </div>
                                </div>
                ...
                <!-- Visibility + stock -->
                <section class="rounded-card border border-line bg-white shadow-sm p-6 sm:p-8 space-y-6">
                    <label class="inline-flex items-center gap-3 cursor-pointer">
                        <input v-model="fields.is_active" type="checkbox" class="h-5 w-5 rounded border-line text-brand" />
                        <span class="text-sm font-semibold text-ink">Product is active (visible to customers)</span>
                    </label>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
                            ...
                            >
                                ...
                                <div v-if="fields.initial_quantity === 0" class="mt-3 shrink-0">
                                    <Link href="/owner/purchase-orders/create" class="inline-flex text-xs font-bold bg-brand-tint text-brand-dark px-3 py-1.5 rounded-control border border-brand-soft hover:bg-brand-tint">
                            ...
                            <div
                                :class="
                                    combinations.length === 0
                                        ? 'flex h-full min-h-0 flex-col rounded-card border border-line bg-mist/70 p-4 sm:p-5'
                                        : 'sm:col-span-2 rounded-card border border-line bg-mist/70 p-4 sm:p-5'
                                "
                            >
