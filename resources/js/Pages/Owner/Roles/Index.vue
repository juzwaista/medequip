    ...
    <OwnerLayout title="Role Management">
        <div class="max-w-7xl mx-auto py-6">
            ...
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h2 class="text-xl font-bold text-gray-900">Staff Roles</h2>
                    <p class="text-sm text-gray-500">Create custom roles and assign specific permissions for your shop's staff.</p>
                </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <tr v-for="role in roles" :key="role.id" class="hover:bg-gray-50/50 transition">
                            <td class="px-6 py-4 font-bold text-gray-900">
                                {{ role.name }}
                                <span v-if="role.name === 'Owner'" class="ml-2 px-2 py-0.5 rounded text-[10px] bg-red-100 text-red-800 uppercase">Shop Owner</span>
                            </td>
                            <td class="px-6 py-4 text-gray-600">
                                {{ role.permissions?.length || 0 }} permissions
                            </td>
                            <td class="px-6 py-4 text-right">
                                <button v-if="role.name !== 'Owner'" @click="openModal(role)" class="text-blue-600 hover:text-blue-800 font-semibold mr-3">Edit</button>
                ...
                <div v-if="modal.open" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
                    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]">
                        <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                            <h3 class="text-lg font-bold text-gray-900">{{ modal.role ? 'Edit Role' : 'Create Custom Role' }}</h3>
                        ...
                        <div class="p-6 overflow-y-auto">
                            <div class="mb-6">
                                ...
                                <input v-model="form.name" type="text" class="w-full border border-gray-300 rounded-lg p-3 text-sm focus:ring-blue-500 focus:border-blue-500" placeholder="e.g. Warehouse Staff, Returns Manager" required>
                                <p v-if="form.errors.name" class="mt-1 text-xs text-red-600">{{ form.errors.name }}</p>
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-gray-700 uppercase mb-3">Allowed Actions</label>
                                <div class="space-y-6">
                                    <div v-for="(perms, group) in permissions" :key="group" class="bg-gray-50 p-4 rounded-xl border border-gray-100">
                                        <h4 class="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3 border-b border-gray-200 pb-2">{{ group }} Permissions</h4>
                                        <div class="grid sm:grid-cols-2 md:grid-cols-3 gap-3">
                                            <label v-for="p in perms" :key="p.id" class="flex items-start gap-2 cursor-pointer">
                                                ...
                                                <span class="text-sm text-gray-700 select-none">{{ p.name.replace('shop.', '') }}</span>
                                            </label>
                                        </div>
                                    </div>
                                </div>
