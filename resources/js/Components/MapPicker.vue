    ...
    <div class="relative w-full rounded-card overflow-hidden shadow-sm border border-line">
        ...
        <div class="absolute top-2 left-1/2 -translate-x-1/2 z-[400] flex gap-2 w-11/12 max-w-sm">
            <button
                ...
                class="flex-1 bg-white text-ink font-semibold text-sm px-4 py-2 rounded-control shadow-md hover:bg-mist flex items-center justify-center gap-2 transition"
                :disabled="isLocating || isGeocoding"
            >
                <svg v-if="isLocating || isGeocoding" class="animate-spin h-4 w-4 text-brand" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
                <svg v-else class="h-4 w-4 text-brand" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
        ...
        <div v-if="locationError" class="absolute top-14 left-1/2 -translate-x-1/2 z-[400] w-11/12 max-w-sm">
            <p class="text-xs text-red-700 bg-red-50/95  px-3 py-2 rounded-control shadow-lg border border-red-200 text-center font-bold animate-bounce">
                ⚠️ {{ locationError }}
            </p>
        </div>

        <div class="absolute bottom-2 left-2 right-2 z-[400]">
            <p v-if="isGeocoding" class="text-[11px] text-brand-dark bg-brand-tint/90  px-2 py-1 rounded shadow-sm text-center ">
                📍 Detecting location...
            </p>
            <p v-else class="text-[11px] text-ink bg-white/90  px-2 py-1 rounded shadow-sm text-center">
