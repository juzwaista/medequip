<template>
    ...
    >
        <div v-if="show" class="fixed top-4 left-4 right-4 sm:left-auto sm:right-4 sm:top-20 z-50 max-w-none sm:max-w-sm w-auto sm:w-full">
            <div 
                :class="[
                    'rounded-control shadow-lg p-4 flex items-start space-x-3',
                    type === 'success' ? 'bg-brand-tint border-l-4 border-brand' : '',
                    type === 'error' ? 'bg-red-50 border-l-4 border-red-500' : '',
                    type === 'info' ? 'bg-brand-tint border-l-4 border-brand' : '',
                    type === 'warning' ? 'bg-yellow-50 border-l-4 border-yellow-500' : ''
                ]"
            >
                <!-- Icon -->
                <div class="flex-shrink-0">
                    <svg v-if="type === 'success'" class="h-6 w-6 text-brand" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <svg v-if="type === 'info'" class="h-6 w-6 text-brand" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div class="flex-1">
                    <p 
                        :class="[
                            'text-sm font-medium',
                            type === 'success' ? 'text-brand-dark' : '',
                            type === 'error' ? 'text-red-800' : '',
                            type === 'info' ? 'text-brand-dark' : '',
                ...
                <button 
                    ...
                    class="flex-shrink-0"
                >
                    <svg class="h-5 w-5 text-ink-soft hover:text-ink" fill="currentColor" viewBox="0 0 20 20">
