<template>
    ...
    <div v-if="needsAcceptance && !dismissed" class="fixed inset-0 z-[90] bg-black/70 flex items-end sm:items-center justify-center p-0 sm:p-4">
        <div class="bg-white w-full sm:max-w-lg sm:rounded-card shadow-2xl overflow-hidden">
            <!-- Top accent -->
            <div class="h-1.5 bg-gradient-to-r from-brand to-brand"></div>

            <div class="p-6">
                <div class="flex items-start gap-4 mb-5">
                    <div class="w-12 h-12 bg-brand-tint rounded-card flex items-center justify-center flex-shrink-0">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-ink">Updated Terms & Conditions</h2>
                ...
                <div class="bg-amber-50 border border-amber-200 rounded-card p-4 mb-5">
                ...
                <div ref="termsContainer" @scroll="onScroll" class="bg-mist border border-line rounded-card p-4 mb-4 max-h-40 overflow-y-auto text-xs text-ink-soft leading-relaxed">
                    <p class="font-bold text-ink mb-2">Summary of Terms</p>
                    ...
                    <p class="mb-2"><strong>For Customers:</strong> You agree to our ordering, delivery, and refund policies. Medical devices requiring prescription must be ordered with valid documentation.</p>
                    ...
                    <button @click="showFull = true" class="text-brand hover:underline font-semibold mt-2 block">Read full Terms & Conditions →</button>
                    <!-- Small spacer at the bottom to ensure scroll threshold is clearly crossed -->
                    <div class="h-2"></div>
                </div>

                <div class="flex items-center gap-2 mb-5">
                    <input type="checkbox" id="acceptCheck" v-model="hasChecked" :disabled="!hasScrolledToBottom" class="rounded border-line text-brand shadow-sm focus:border-brand-soft focus:ring focus:ring-brand-soft focus:ring-opacity-50 transition-opacity" :class="{'opacity-50 cursor-not-allowed': !hasScrolledToBottom}">
                    <label for="acceptCheck" class="text-sm text-ink transition-opacity" :class="{'opacity-50': !hasScrolledToBottom}">I have read and accept the Terms & Conditions</label>
                </div>
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <button @click="logout" class="w-full text-ink-soft font-semibold py-2 text-sm hover:text-ink transition">
                        Decline & Sign Out
                    </button>
                </div>
            </div>
